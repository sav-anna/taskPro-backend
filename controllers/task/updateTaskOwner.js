...
const updateTaskOwner = async (req, res) => {
  ...
  const currentColumn = await Column.findByIdAndUpdate({ _id: currentOwnerId }, { taskOrder: orderForCurrentColumn }, { new: true });
  if (!currentColumn) throw HttpError(404);

  if (newOwnerId !== currentOwnerId) {
    const updatedTask = await Task.findByIdAndUpdate(taskId, { parentColumn: newOwnerId }, { new: true });
    const newColumn = await Column.findByIdAndUpdate({ _id: newOwnerId }, { taskOrder: orderForNewColumn }, { new: true });
